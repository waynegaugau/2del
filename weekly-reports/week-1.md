# Báo cáo Tuần 1
**Tuần:** 1 01.03.2026 - 07.03.2026  
**Nhóm:** 8  
**Đề tài:** 18 - Hệ thống Phòng Khám Thú Y 2DEL  
**Nhóm trưởng:** Phan Nguyễn Linh Năng - 2251012099  
---
## 1. Công việc đã hoàn thành
| Thành viên | MSSV | Công việc | Link Commit/PR |
|------------|------|-----------|----------------|
| Phan Nguyễn Linh Năng | 2251012099 | Thiết kế database | [abc123](link) |
| Hồ Ngọc Minh | 2251012092 | API User Service | [def456](link) |
---
## 2. Tiến độ tổng thể
| Hạng mục | Trạng thái | % |
|----------|------------|---|
| Phân tích yêu cầu | Done | 100% |
| Thiết kế kiến trúc | Done | 100% |
| Backend API | Đang làm | 50% |
| Frontend UI | Đang làm | 30% |
| Docker | Chưa bắt đầu | 0% |
| Testing | Chưa bắt đầu | 0% |
**Tổng tiến độ: ___%**
---
## 3. Kế hoạch tuần tới
| Thành viên | Công việc dự kiến |
|------------|-------------------|
| Nguyễn Văn A | Hoàn thành Order Service |
| Trần Thị B | API Product Service |
| Lê Văn C | Frontend Dashboard |
| Phạm Văn D | Docker Compose |
---
## 4. Khó khăn / Cần hỗ trợ
- [ ] [Mô tả khó khăn nếu có]
- [ ] [Câu hỏi cần GV giải đáp nếu có]
---
*Ngày nộp: 07.03.2026*
*Xác nhận của Nhóm trưởng: [Phan Nguyễn Linh Năng]*
