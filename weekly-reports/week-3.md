# Báo cáo Tuần 3
**Tuần:** 3 (16.03.2026 - 22.03.2026)  
**Nhóm:** 8  
**Đề tài:** 18 - Hệ thống Phòng Khám Thú Y 2DEL  
**Nhóm trưởng:** Phan Nguyễn Linh Năng - 2251012099
---
## 1. Công việc đã hoàn thành
| Thành viên | MSSV | Công việc | Link Commit/PR |
|------------|------|-----------|----------------|
| Phan Nguyễn Linh Năng | 2251012099 | Khởi tạo frontend, khảo sát thực tế | [efc4981](https://github.com/waynegaugau/2del/commit/efc498123f9c80367148cf98841568836fca3567) |
| Hồ Ngọc Minh | 2251012092 | Khởi tạo backend, cấu hình và kết nối database | [aa1a0fe](https://github.com/waynegaugau/2del/commit/aa1a0fe9a11ad2f8034f6de146d533a20b8b593e) |
---
## 2. Tiến độ tổng thể
| Hạng mục | Trạng thái | % |
|----------|------------|---|
| Phân công | Hoàn thành | 100% |
| Phân tích yêu cầu | Đang làm | 96% |
| Thiết kế kiến trúc | Đang làm | 67% |
| Backend API | Đang làm | 10% |
| Frontend UI | Đang làm | 7% |
| Docker | Chưa bắt đầu | 0% |
| Testing | Chưa bắt đầu | 0% |
**Tổng tiến độ: 36.18%**
---
## 3. Kế hoạch tuần tới
| Thành viên | Công việc dự kiến |
|------------|-------------------|
| Phan Nguyễn Linh Năng | Kiểm tra các luồng hoạt động, giao diện, xem lại phần test plan |
| Hồ Ngọc Minh | Tiếp tục thiết kế backend và API, kiểm tra kiến trúc cơ sở dữ liệu |
---
## 4. Khó khăn / Cần hỗ trợ
- Các đầu công việc hơi nhiều, các mục tiêu cần được cụ thể chi tiết hơn.
- Các luồng dữ liệu, bảng và cơ sở dữ liệu đang bị rối, cần tinh gọn và tối giản hơn cho hiệu quả.
---
*Ngày nộp: 22:54 22.03.2026*  
*Xác nhận của Nhóm trưởng: Phan Nguyễn Linh Năng*
